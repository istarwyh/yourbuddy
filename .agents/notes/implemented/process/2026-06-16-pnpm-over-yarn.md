...
- **Build scripts.** `pnpm-workspace.yaml` sets `dangerouslyAllowAllBuilds: true`, so installing the repository dependencies also runs their lifecycle scripts without a separate approval step. `peerDependencyRules.allowedVersions.typescript: '>=5 <7'` silences benign peer-range warnings for the in-repo TypeScript.
