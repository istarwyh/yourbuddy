...
- **构建脚本。** `pnpm-workspace.yaml` 设置 `dangerouslyAllowAllBuilds: true`，因此安装仓库依赖时会直接运行生命周期脚本，无需单独审批。`peerDependencyRules.allowedVersions.typescript: '>=5 <7'` 消除仓库内 TypeScript 的良性 peer 范围警告。
