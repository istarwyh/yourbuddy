...
`job_output` 在 `tools/pre-execute` 宣告它正在收集确切的所有者与任务，这发生在策略或注册表等待开始之前，并在 `tools/result` 清除该声明。落在这个区间内的完成会被延迟，而不是立即注入。最后一个匹配执行结束后，报告方读取注册表的权威 `reported` 位：成功的终态读取已经领取结果，不需要重复通知；任何拒绝或失败则仍让通知可投递。这关闭了一个调度窗口：结算可能发生在工具调用已对模型可见之后，却早于 `JobRegistry.wait()` 注册 waiter。

该声明按确切所有者和任务计数，因为两个并发读取中只要还有一个可能报告终态，就不能释放通知。记录已经消失表示所有者释放已移除其读取者，因此延迟通知随之丢弃。
