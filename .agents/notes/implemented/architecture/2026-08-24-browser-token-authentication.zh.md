...
**把不含认证的干净根路径 401 视为桌面就绪。** 这只能证明 Socket 会响应。全新 WebView 会在没有 Session Cookie 的情况下加载同一个干净 URL 并显示 401 响应，因此原生启动必须先完成交换并安装经过校验的 Session Cookie，再让 Shell 打开不含凭据的 Host 根地址。
