...
The YourBuddy desktop supervisor accepts the printed URL only when it uses HTTP, the selected `127.0.0.1` port, root path, and one bounded base64url token. It continues draining stdout after that line, verifies a reachable 303 response with both the clean-root location and session cookie, and preserves the URL only for the embedded WebView's first navigation. The credential-free root remains the separately stored origin and diagnostic URL. Native and WSL startup retain child-exit and total-timeout handling; the WSL exchange also proves that Windows localhost forwarding reaches the Linux Host. Startup logs and failure tails redact a token if an upstream diagnostic repeats it.
...
**Treat an unauthenticated 401 from the clean root as desktop readiness.** This proves only that the socket responds. A fresh WebView would load the same clean URL without a session cookie and display the 401 response, so the desktop must retain and navigate through the authenticated startup URL.
