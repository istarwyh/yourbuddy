...
The YourBuddy desktop supervisor accepts the printed URL only when it uses HTTP, the selected `127.0.0.1` port, root path, and one bounded base64url token. It continues draining stdout after that line, completes the reachable 303 exchange, validates and retains only the returned session cookie, and discards the URL before creating the main WebView. The credential-free root remains the separately stored origin and diagnostic URL. Native and WSL startup retain child-exit and total-timeout handling; the WSL exchange also proves that Windows localhost forwarding reaches the Linux Host. Startup logs and failure tails redact a token if an upstream diagnostic repeats it. YourBuddy's same-site shell and WebView cookie handoff are owned by the [desktop authentication fix](../bug-fix/2026-09-06-yourbuddy-desktop-same-site-authentication.md).
...
**Treat an unauthenticated 401 from the clean root as desktop readiness.** This proves only that the socket responds. A fresh WebView would load the same clean URL without a session cookie and display the 401 response, so native startup must complete the exchange and install the validated session cookie before the shell opens the clean Host root.
