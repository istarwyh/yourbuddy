...
function isMaintained(file: string): boolean {
  return file !== 'apps/desktop-tauri/product/DSH_UPSTREAM.json'
    && !excludedPrefixes.some(prefix => file.startsWith(prefix))
    && !/^docs\/releases\/yourbuddy-v(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\//u.test(file)
}
 ...
 * @returns Prohibited references outside frozen sources and versioned release archives; absent shallow-history objects cannot match.
 */
