...
describe('gate graph validation', () => {
  ...
  it('keeps the hygiene aggregate aligned with the package script checks', () => {
    ...
    expect(ids).toEqual([
      ...
      'optional-dependency-imports', 'client-packages', 'client-ui-i18n',
      'desktop-shell-permissions', 'cordis-config', 'runtime-closure', 'vendored-links',
    ])
  ...
  it.each(['ci-primary', 'ci-static', 'check-all', 'hygiene'] as const)(
    'keeps desktop shell permission parity in %s',
    (mode) => {
      const ids = withPnpmEntrypoint(() => gatesForMode(mode).map(subject => subject.id))

      expect(ids).toContain('desktop-shell-permissions')
    },
  )
...
describe('process-table parsing', () => {
  ...
  it('walks each descendant once when process rows contain duplicates or a cycle', () => {
  ...
  it('walks a wide process table without spreading the child list onto the call stack', () => {
    const rows = Array.from({ length: 100_000 }, (_, index): [number, number] => [index + 1_000, 100])

    expect(collectDescendants(100, rows)).toHaveLength(rows.length)
  })
})
