...
import { readPackageGroupOrder } from './verify-package-group-map.ts'
 ...
 * @param groupOrder - Reader-facing package-group order.
...
export function computeModuleGraphOutputs(scanRoot: string = root): ReadonlyMap<string, string> {
  const groupOrder = readPackageGroupOrder(scanRoot)
  const packages = collectPackageGraph(scanRoot, groupOrder, 'gen-module-graph')
  return new Map([
    [PATHS.source, renderModuleGraph(packages, 'en', groupOrder)],
    [PATHS.zh, renderModuleGraph(packages, 'zh', groupOrder)],
  ])
}
