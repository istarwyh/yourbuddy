...
const externalProductSnapshots = [
  'context-doctor',
  'dsh-better-sidebar',
  'dsh-codex-auth',
  'harbor-evolution',
...
describe('maintained repository reference policy', () => {
  ...
  it('excludes immutable YourBuddy inputs without excluding maintained product and release files', () => {
    for (const directory of externalProductSnapshots) {
      expect(findRepositoryReferences(
        `apps/desktop-tauri/product/${directory}/nested/source.md`,
        organizationUrl,
        new Set(),
      )).toEqual([])
    }
    expect(findRepositoryReferences(
      'apps/desktop-tauri/product/DSH_UPSTREAM.json',
      organizationUrl,
      new Set(),
    )).toEqual([])
    expect(findRepositoryReferences(
      'docs/releases/yourbuddy-v1.2.3/evidence/source.txt',
      organizationUrl,
      new Set(),
    )).toEqual([])

    for (const file of [
      'apps/desktop-tauri/product/personal-workbench/source.md',
      'apps/desktop-tauri/product/DSH_UPSTREAM.json.backup',
      'docs/releases/README.md',
      'docs/releases/_template/README.md',
      'docs/releases/yourbuddy-v1.2.3-rc.1/README.md',
    ]) {
      expect(findRepositoryReferences(file, organizationUrl, new Set()))
        .toEqual([{ file, line: 1, kind: 'organization-url' }])
    }
  })
