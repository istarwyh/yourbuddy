...
YourBuddy 0.3.14 makes creator publishing available from the built-in Content Creation preset.
...
- Product snapshot hashing now uses locale-independent UTF-8 path ordering and ignores non-executable permission variation while retaining executable-bit integrity, correcting the hosted build failure recorded for the unreleased 0.3.13 artifacts.
...
YourBuddy 0.3.14 在内置的“内容创作”Preset 中提供创作者发布能力。
...
- 产品快照哈希改用与 Locale 无关的 UTF-8 路径顺序，并忽略非可执行权限差异，同时保留可执行位完整性，从而修正未发布 0.3.13 产物中记录的托管构建失败。
...
- [Tagged verification record / Tag 内验证记录](https://github.com/istarwyh/yourbuddy/tree/yourbuddy-v0.3.14/docs/releases/yourbuddy-v0.3.14)
