/** Refresh YourBuddy's external product snapshots from their declared latest channels. */
...
export function validateProductUpdatePolicy(
  ...
  if (!isPlainObject(value)) throw new Error(`unsupported YourBuddy product update policy: ${policyPath}`)
  ...
  if (value.formatVersion !== 1
    ...
    throw new Error(`unsupported YourBuddy product update policy: ${policyPath}`)
  }
...
/**
 * Read and fully validate YourBuddy's product update policy.
 *
 * @param {string} selectedProductRoot
 * @returns {Record<string, unknown>}
 */
export function readProductUpdatePolicy(selectedProductRoot = productRoot) {
  ...
  try {
    ...
    throw new Error(`cannot read YourBuddy product update policy ${selectedPolicyPath}: ${error.message}`)
  }
...
function requestHeaders(url) {
  const headers = { 'user-agent': 'yourbuddy-release-preparation' }
...
async function stageGitHubReleasePair(policy, roots, fetchImpl) {
  ...
  if (current.provenance?.commit === latest.commit && current.provenance?.releaseTag === latest.tag) {
    ...
    if (compatibilityPatches.length > 0) {
      const patches = [
        ...
        'Preserve the YourBuddy bilingual README projection.',
      ]
  ...
  writeProvenance(stagedPython, {
    ...
    patches: ['Preserve the YourBuddy bilingual README projection.'],
    license: manifest.license,
  })
  return [
...
function applyUpdates(updates) {
  const transaction = mkdtempSync(join(tmpdir(), 'yourbuddy-product-rollback-'))
  const backups = []
  try {
  ...
  try {
    for (const plugin of policy.plugins) {
      if (plugin.kind === 'npm-latest') {
        ...
        throw new Error(`unsupported YourBuddy product update kind: ${plugin.kind}`)
      }
    }
