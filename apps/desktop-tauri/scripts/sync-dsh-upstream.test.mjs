...
test('DSH sync prepares one upstream merge and rollback restores every owned file', async (t) => {
  ...
  const upstreamRecord = JSON.parse(readFileSync(join(fixture.product, 'DSH_UPSTREAM.json'), 'utf8'))
  assert.equal(upstreamRecord.version, nextVersion)
  assert.equal(upstreamRecord.commit, nextCommit)
  assert.deepEqual(upstreamRecord.patches, [{ id: 'workbench-layout', file: 'patches/dsh-workbench-layout.patch' }])
  assert.equal(mergeOpen, true)
