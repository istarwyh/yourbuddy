...
这个第一方 YourBuddy 产品插件负责三个通用设置卡片。用户可以在**设置 → 通用设置 → 我的工作台**中替换侧边栏工作台名称、Logo、新会话标题与标题徽章，预览草稿、隐藏徽章并恢复 YourBuddy 默认值。**设置 → 通用设置 → 网络代理**用于配置应用全局的直连、跟随 macOS 系统代理或自定义代理策略，通过 ChatGPT 分别测试原生草稿链路、新启动的托管 Node 草稿链路与当前 Node Host 链路，并且只有两条草稿链路都通过时才持久化选择，再重启 YourBuddy。**设置 → 通用设置 → 应用生命周期**会请求受信任的桌面 Shell 运行带签名的应用更新器，或在停止私有 Host 后重启整个应用。重启会加载通过 Plugin Marketplace 安装的插件。桌面操作在独立的 `dsh web` 中保持可见但不可用；浏览器请求不能选择任意 Tauri Command，代理配置不接受凭据，更新操作也不会在最终用户机器上刷新插件源码，因为内置产品插件会随带签名的应用 Release 一起升级。

这个插件还负责协调桌面端的中间工作台与完整的右侧会话区域。Better Sidebar 会保持挂载，并作为默认的中间界面；选择创作者内容时，其详情会临时显示在同一个位置。Conversation 标题栏与中间区域边缘的控件都可以把原生右侧栏和 Conversation 一并收起为零宽轨道；收起后，中间控件会按此前的正宽度恢复它们。选择另一个当前 Session 会回到 Better Sidebar 并展开会话区域；当前 Session 请求输入或持久化的 Turn 完成时，Conversation 也会自动恢复。后台资源打开不会替换正在显示的内容详情。
...
桌面应用会把平台窗口控件放在侧栏标题行的折叠按钮之前。macOS 使用一组紧凑横排的红黄绿按钮，不添加分隔线；其他平台保留各自的操作顺序和图标。浏览器组件只能通过桌面 Shell 请求固定的最小化、最大化、关闭与拖动操作。启动、重载或侧栏收起时，Shell 会用一个小型浮动回退入口提供相同操作，不再预留整条纵向控制栏。
...
浏览器会把上传图片保存为当前 Profile 中的 data URL。Hero 文本字段留空时使用本地化默认值，`showHeroBadge: false` 会隐藏徽章。品牌设置不会改变浏览器标题、可执行文件名、应用图标、主题或其他 UI 文案。
...
默认名称为 YourBuddy，默认 Logo 为随附的 Y8 图标。Conversation Shell 提供本地化的新会话标题与徽章默认值。自定义值缺失或停用时恢复这些默认值；卸载插件后才恢复上游 Shell 的默认呈现。
