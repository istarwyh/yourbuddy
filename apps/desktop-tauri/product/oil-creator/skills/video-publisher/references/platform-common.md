...
Never click a final `发布`, `发布笔记`, `发表`, `立即投稿`, `定时发布`, `定时发表`, `保存`, `安排时间`, `Save`, `Publish`, or `Schedule` button. Final publishing is outside this Skill. Every adapter result must leave `finalPublishClicked: false`. The shared core keeps its capture-phase click/submit guard armed through complete `inspect` or `verify` evidence, then detaches it only for a successful user handoff and verifies removal with inert synthetic controls. `READY` requires `guardArmed: true` during evidence capture, zero blocked attempts, and a proven detach; incomplete, blocked, failed-detach, and user-control results do not detach the guard.
