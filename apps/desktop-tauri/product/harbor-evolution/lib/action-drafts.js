  ...
  async schedule(entry, basis, owner) {
    ...
    task.promise = (async () => {
      try {
        ...
        await record('EXECUTING')
        const result = await this.execute(entry.draft, basis, owner, { plan: entry.prepared.plan, operationId, signal: abort.signal, onUsage: readUsage => { if (typeof readUsage === 'function') task.readUsage = readUsage }, onSpawn: (pid, checkpoint) => record('ACTIVE', { jobName: checkpoint.job, diagnosticOnly: true, processStarted: true, ...(Number.isSafeInteger(pid) && pid > 1 && checkpoint.process?.groupId === pid ? { process: { pid, groupId: pid, platform: checkpoint.process.platform, ...(checkpoint.process.executionEnvironment === 'host' && checkpoint.process.dockerTransport === 'none' ? { executionEnvironment: 'host', dockerTransport: 'none' } : checkpoint.process.dockerTransport === 'pinned-local-unix/v1' ? { executionEnvironment: 'docker', dockerTransport: checkpoint.process.dockerTransport } : {}), ...Object.fromEntries(['hostIdentity', 'dockerIdentity'].filter(key => /^sha256:[a-f0-9]{64}$/.test(checkpoint.process[key] ?? '')).map(key => [key, checkpoint.process[key]])) } } : {}) }) })
