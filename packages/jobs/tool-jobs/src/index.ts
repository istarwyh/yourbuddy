...
export function apply(ctx: Context, config: Config): void {
  ...
  interface ActiveOutputRead {
    executions: Set<ToolExecution>
    collectedTerminal: boolean
  }
  const activeOutputReads = new Map<Agent['id'], Map<JobId, ActiveOutputRead>>()
  const outputReadByExecution = new WeakMap<ToolExecution, {
    owner: Agent['id']
    id: JobId
    reads: Map<JobId, ActiveOutputRead>
    active: ActiveOutputRead
  }>()
  const deferredCompletions = new Map<Agent['id'], Map<JobId, JobView>>()
  ...
  const beginOutputRead = (exec: ToolExecution): void => {
    ...
    const reads = activeOutputReads.get(owner) ?? new Map<JobId, ActiveOutputRead>()
    const active = reads.get(id) ?? { executions: new Set<ToolExecution>(), collectedTerminal: false }
    active.executions.add(exec)
    reads.set(id, active)
  ...
  const deliverCompletion = (job: JobView): void => {
    ...
    const message = createUserMessage({
      content: [{
        type: 'text',
        text: fitCompletionNotice(job),
      }],
      source: {
        kind: 'tool-jobs',
        form: 'notice',
        summary: completionSummary(job),
      },
    })
  ...
  const finishOutputRead = (exec: ToolExecution): void => {
    ...
    read.active.executions.delete(exec)
    if (read.active.executions.size > 0) return
  ...
  ctx.jobs.events.subscribe({ owners: 'scope' }, (event) => {
    if (event.type === 'removed') {
      ...
      if (event.job.owner !== undefined) {
        const completions = deferredCompletions.get(event.job.owner)
        completions?.delete(event.job.id)
        if (completions?.size === 0) deferredCompletions.delete(event.job.owner)
      }
      return
    }
    ...
    const active = activeOutputReads.get(event.job.owner)?.get(event.job.id)
    if (active !== undefined) {
      const completions = deferredCompletions.get(event.job.owner) ?? new Map<JobId, JobView>()
      completions.set(event.job.id, event.job)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const value = readBody(jobs.read(id, exec.agent?.id))
      const active = outputReadByExecution.get(exec)?.active
      if (active !== undefined && value.job.status !== 'running' && value.job.status !== 'stopping') {
        active.collectedTerminal = true
      }
      return value
    },
