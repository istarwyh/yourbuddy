...
席位通过 `ctx.layout.openRightbar(track, fullscreen)` / `closeRightbar()` 报告呈现，框架不注入本包。框架的聚合 Session 区域包装可以隐藏面板，而不改变其报告、每会话布局、展开状态或保留的 tab 正文；恢复该区域后会显示同一呈现。宽屏切换全屏不改变中栏宽度；宽度拖拽区只在普通展开态显示。独立浮窗及 `float`/`dock` 操作保持可用。
...
Tab 域按（Session，Tab id）保留品牌化 occurrence id、导航、中止信号与绑定动作。私有装配回调采用各 Session 的 store，立即对齐恢复的记录，再跟随其提交。同一 Session 的 store 重建会替换其 `adopt` 注册，同时释放旧订阅与清理引用。记录消失或插件卸载才中止 signal，收起和切会话不销毁记录；undo 恢复的是新 occurrence。`useTabInfo()` 组合框架绑定的 store 与导航 hook，不在组件中手写订阅或在渲染时创建记录。`tab.actions` 始终作用于自己的会话；`toSide: true` 会先按普通分栏上限与空间规则严格创建右侧格，无法分栏时不打开内容，这与 `preferNewPane` 的回退行为不同。`tab.visible` 区分正文与标题，并排除后台 Session；前台浮窗不受整栏收起影响。Store adoption 保持私有；公开 Controller 只暴露采用完成通知。
