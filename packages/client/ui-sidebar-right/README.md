...
The seat reports its own presentation through `ctx.layout.openRightbar(track, fullscreen)` / `closeRightbar()`; the frame does not inject this package. The frame's aggregate `visible` owner prop may suppress the docked panel and floating host without changing that report, the per-Session expanded state, or mounted tab content; restoring it reveals the same presentation. Switching fullscreen on a wide viewport leaves the center width unchanged, and the width handle appears only in expanded normal mode. Independent floating panels and `float`/`dock` operations remain available.
