...
The seat reports presentation through `ctx.layout.openRightbar(track, fullscreen)` / `closeRightbar()`; the frame does not inject this package. The frame's aggregate Session-region wrapper can hide the panel without changing its report, per-Session layout, expanded state, or retained tab bodies; restoring the region reveals the same presentation. Switching fullscreen on a wide viewport leaves the center width unchanged, and the width handle appears only in expanded normal mode. Independent floating panels and `float`/`dock` operations remain available.
