...
import type { PanelInfo, WorkbenchLayoutState } from './service.ts'
...
export type {
  ILayout, MainPanelId, PanelInfo, WorkbenchLayoutBinding, WorkbenchLayoutSnapshot,
} from './service.ts'
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => {
    ...
    const workbenchLayout: HostObservable<WorkbenchLayoutState> = {
    ...
    const disposeRegistration = ctx.slots.register({
      ...
      store,
      inject: () => ({
        hooks: { workbenchLayout },
        setWorkbenchWidth: (width: number) => { layout.setWorkbenchWidth(width) },
      }),
    }, AppFrame)
