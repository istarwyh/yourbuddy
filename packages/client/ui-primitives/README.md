...
`projectUserText(text, sessionLabels)` decorates sent user references. Pass separate text blocks to display automatic page context as a collapsed attachment with its provider-supplied label. Expanding shows its frozen `description` when present, otherwise its literal body; metadata is rendered as text, never HTML. The first block is always user-authored text (empty for image-only sends); only later complete `dsh-page-context` envelopes become attachments. Scalar input, inline envelopes, and malformed envelopes remain user text. `projectUserText(text, sessionLabels, 'editable')` returns only authored text for copying or editing, preserving whitespace and reference wire forms. Both projections leave the model input and durable log unchanged.
