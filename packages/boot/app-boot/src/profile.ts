...
function readOptionalPackageManifest(anchor: string): ProfileManifest | undefined {
  try {
...
function collectInstallationScopePackages(
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    ...
    for (const dep of profileDependencyNames(next.manifest)) {
      ...
      try {
        manifest = skippedBundles.has(dep)
          ? readProfileManifest('dsh', dir)
          : readOptionalPackageManifest(manifestPath)
      } catch (error) {
  ...
  for (const anchor of anchors) {
    ...
    for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
      ...
      for (const dep of profileDependencyNames(next.manifest)) {
        ...
        if (dir === undefined) continue
        const manifestPath = join(realModuleDirectory(dir), 'package.json')
        const dependencyManifest = readOptionalPackageManifest(manifestPath)
