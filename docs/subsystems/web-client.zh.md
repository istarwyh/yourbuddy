...
`ui-slots` 提供类型化 registry 与 lifecycle ledger；`ui-renderer` 是唯一通过 `useSyncExternalStore` 绑定裸 observable、拥有 React context 并渲染 root tree 的包。功能 component 通过推导出的 props 接收 framework hook、owner prop、store action 与显式 injection。`ui-layout` 可以让已占用的 workbench 控制一个包含选中主内容与 rightbar 的辅助区域，同时让各 occupant 的状态留在所属包中。[Web Client Slots](slots.zh.md)列出这些输入、扩展 API 与当前 Slot 层级。
