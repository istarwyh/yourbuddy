...
Date: 2026-09-06. A separate checkout holds a fresh offline dependency installation and build. It includes the current worktree changes for integration testing while preserving the original checkout's unrelated desktop-authentication work and old build artifacts.
...
`pnpm run test:docs` passed all 15 checks; `pnpm run doc-sync` passed all 32, including the SDK documentation build and site checks. The release-readiness rerun first found that the new Host Web scenario was absent from the Host TypeScript aggregate and remained in the Web client aggregate; the build and lint therefore rejected its cross-Realm imports. Adding the scenario to `tsconfig.host.json` and excluding it from `apps/web/tsconfig.json` made the clean repository build, full lint, and the 2-case Web scenario pass. `git diff --check` passed.
...
No installer or website was published in this task. The release maintainer must deploy the matching documentation with the next application release through the [website synchronization procedure](../../product-website.md#release-synchronization), then check the public Help destinations.
