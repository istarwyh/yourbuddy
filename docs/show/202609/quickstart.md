...
This tutorial helps a new user complete a task in an explicit working directory and locate its actual output. It targets the macOS Apple Silicon desktop release of YourHarness. Before publishing the page, follow these steps with the corresponding installer and add screenshots.

## 1. Install and launch

Choose an installer from [YourHarness Releases](https://github.com/istarwyh/yourharness/releases) that explicitly supports your system, mount the DMG, and install the application. The download page also shows the version, architecture, checksums, and known limitations. Do not derive an availability promise from the source version when the corresponding artifact does not exist.
...
> Create hello-yourharness.md in the current working directory, with three things to complete this week and acceptance criteria for each. When finished, tell me the file location and summarize what you wrote.
...
| A newly installed plugin has no interface | Confirm installation into YourHarness's Web Profile, restart, and check whether the plugin loaded |
