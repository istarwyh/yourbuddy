...
发布后，从 0.3.11 GitHub Release 安装 Apple Silicon DMG，或在旧版 YourBuddy 中使用**设置 → 通用设置 → 应用生命周期 → 检查更新**。公开文件完成独立核验之前，0.3.10 仍是已核验下载版本。
...
| 正式桌面发布 | pending | 不可变 Tag `yourbuddy-v0.3.11` | GitHub Actions macOS 15 arm64 | 待定 |
| 公开安装包、Updater、App 标识、Provenance 与迁移运行时 | pending | 公开 Release 文件 | GitHub Release 与 macOS arm64 | 待定 |
| 产品网站与可下载验证归档 | pending | 公开部署与归档 | GitHub Pages 与 GitHub Release | 待定 |
...
- 产品发布状态：等待 `yourbuddy-v0.3.11` Tag 与 GitHub Actions 发布；0.3.10 仍是已核验公开版本。
- 验证归档状态：发布前部分归档；本地候选证据已包含，CI、公开产物、网站与可下载归档证据仍待完成。
- 网站同步状态：待定；候选文档已经准备，但不会替换已核验的 0.3.10 下载。
- 未验证范围：原生 GUI 启动、安装包 WebView 交互、从旧安装更新、Apple Developer 签名和公证、OAuth、可选创作集成、真实模型流量，以及全部公开文件声明。
...
- [x] 双语发布索引已包含本版本。
- [ ] 最终候选 Commit 与不可变 Release Tag 已记录。
- [ ] 公开 Release 页、安装包、Updater 元数据、签名、Hash、App 标识、Provenance 与迁移运行时已独立核验。
- [ ] 网站已完成双语同步，并独立核验线上下载旅程。
- [ ] 可下载验证归档已经创建、上传、解压与检查。
