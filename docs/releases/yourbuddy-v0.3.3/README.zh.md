...
8. 检查运行 `34049354876`：四个 Runtime 载体、修正后的浏览器时区与 Inspector 路径均通过；工作流导航测试则在子会话提示词完成刷新前打开了刚进入列表的子会话，真实 pwsh 启动也在受控提示符到达前接受了 stdin 等待证据。
9. 让工作流导航等待子会话提示词持久化检查点；让 pwsh 启动保留提示符证据与最近一次非空启动输出，直到同时观察到受控提示符与 stdin 就绪。
...
- 验证资料归档状态：在已记录的证据范围内完成；公开 ZIP 已下载、完成完整性检查、解压并打开，Release 页面已链接到不可变证据 Commit `1078e7a6690ce2a9f745832c64ecf93d3ef6c474`。原生 UI 自动化服务失败导致正式窗口截图仍不可用，该项如实保留为未验证的可视化范围，没有用合成图片替代。
...
- [x] 公开发布页固定链接到不可变证据 Commit 与下载地址；由于没有取得可信的正式窗口截图，不宣称存在图集。
