...
桌面目标为 macOS Apple Silicon。YourBuddy 0.3.3 正在准备发布以取代 0.3.2，目前还没有公开下载。
...
安装后的 0.3.2 应用存在已确认的启动认证缺陷，可能显示 `dsh web authentication required`。除非需要测试历史版本，否则请等待完成独立验证的 0.3.3 下载。
...
0.3.2 Release 提供精确安装包、SHA-256 校验和、带签名的 Tauri Updater 包与验证记录，但启动缺陷仍然存在。0.3.3 发布并完成独立检查后，已有安装可以使用**设置 → 通用设置 → 应用生命周期 → 检查更新**。Updater 签名与 Apple 应用签名相互独立：这些构建使用 ad-hoc 签名，尚未使用 Apple Developer 身份完成签名和公证，因此 Gatekeeper 会拒绝，首次启动可能需要按文档执行 macOS 放行操作。
