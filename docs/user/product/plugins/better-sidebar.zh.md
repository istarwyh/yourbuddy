...
桌面宽度下，在 DSH 对话左侧使用主工作区；拖动分隔条调整两侧宽度，在标签页中打开文件和预览，并通过终端或 Git 面板检查代码变化。窗口较窄时，对话仍是主界面，Better Sidebar 通过抽屉打开；浮动窗口和底部工作区继续可用。推荐插件目标、内嵌浏览器的外部打开操作以及终端中的 HTTP(S) 链接会在系统浏览器中打开。在 YourBuddy 中，只需运行一次推荐插件的安装命令；其隔离 Web Profile 会执行依赖构建脚本，无需另行运行 `pnpm approve-builds`。
